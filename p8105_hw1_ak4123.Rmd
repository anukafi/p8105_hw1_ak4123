...
### _C. Scatterplots of y vs x_
```{R}

ggsave("scatter_plot.pdf", height = 4, width = 6)
```
Color Scale: Green is x+y is greater than 0 and Red is x+y is not greater than 0 
Note: This first plot was saved using ggsave function
```{R}
ggplot(question2_df, aes(x = x, y = y, color = vec.numeric)) + geom_point()
```
Color Scale: The colors are a range, the darkest blue/black is 0.00 and the lightest blue is 1.00
```{R}
ggplot(question2_df, aes(x = x, y = y, color = vec.factor)) + geom_point()
```
Color Scale: Green is True, Red is False  
